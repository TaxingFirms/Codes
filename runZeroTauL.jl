...
#tau = init_taxes(ttaud = 0.15, ttauc = 0.35, ttaui = 0.28, ttaug = 0.15, ttaul = 0.00);
...
#save("ResultsZeroTauL.jld","pr",pr,"eq",eq,"tau",tau,"pa",pa);

pr,eq,pa = tauload("ResultsZeroTauL.jld","pr","eq","tau","pa");
include("TaxReforms.jl")
include("Reforms.jl")

taucvec = [0.33 0.31 0.29 0.27 0.25 0.23 0.21 0.19 0.17];
ref1 = Reform1Vector("ref1ZeroTauL.jld", taucvec, pr, eq, tau, pa; bctol= 5*10.0^-3.0, update=0.75);
